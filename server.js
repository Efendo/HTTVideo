...
app.get('/', async (req, res) => {
    if(!req.query.watch){
        ...
        if (!isXSS(req.query.watch)) {
            res.send(`<link rel="icon" type="image/x-icon" href="/favicon.svg"><title>${Videos.names[Videos.files.indexOf(req.query.watch)]}</title><link rel="stylesheet" href="/watch.css"><video src="/${req.query.watch}" style="width: 100%; height: 100%; margin: 0px;" controls />`);
        } else {
